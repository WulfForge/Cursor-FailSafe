...
export class UI {
    ...
    // Sidebar provider removed - dashboard is now full webview only
    ...
    private readonly realTimeEvents: any[] = [];
    private previewPanel: vscode.WebviewPanel | null = null;
    private previewSyncEnabled = true;
    ...
    constructor(projectPlan: ProjectPlan, taskEngine: TaskEngine, logger: Logger, context?: vscode.ExtensionContext) {
        ...
        // Initialize with sample action log data for testing charts
        this.actionLog = [
            { timestamp: new Date().toISOString().split('T')[0] + 'T10:00:00Z', description: 'Project initialized' },
            { timestamp: new Date().toISOString().split('T')[0] + 'T10:15:00Z', description: 'First task completed' },
            { timestamp: new Date().toISOString().split('T')[0] + 'T11:30:00Z', description: 'Chart system implemented' },
            { timestamp: new Date().toISOString().split('T')[0] + 'T14:20:00Z', description: 'Validation rules added' },
            { timestamp: new Date().toISOString().split('T')[0] + 'T16:45:00Z', description: 'Dashboard updated' }
        ];
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            const actionsCount = Math.floor(Math.random() * 5) + 1; // 1-5 actions per day
            
            for (let j = 0; j < actionsCount; j++) {
                const hour = 9 + Math.floor(Math.random() * 8); // 9 AM to 5 PM
                const minute = Math.floor(Math.random() * 60);
                this.actionLog.push({
                    timestamp: `${dateStr}T${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}:00Z`,
                    description: `Sample action ${j + 1} on day ${i}`
                });
            }
        }
    ...
    private handleRealTimeEvent(event: any): void {
        ...
        // Sidebar removed - no refresh needed
    }
    ...
    private registerCommands(): void {
        ...
        this.disposables.push(
            vscode.commands.registerCommand('failsafe.openPreview', () => {
        ...
        this.disposables.push(
            vscode.commands.registerCommand('failsafe.togglePreviewSync', () => {
                this.previewSyncEnabled = !this.previewSyncEnabled;
                vscode.window.showInformationMessage(
                    `Preview sync ${this.previewSyncEnabled ? 'enabled' : 'disabled'}`
                );
            })
        );
    }
    ...
    private async generateWebviewContent(dashboard: UIDashboard, planValidation: any): Promise<string> {
        ...
        <script>
            // Initialize charts with real data and error handling
            ...
            function showChartError() {
                ...
                chartContainers.forEach(container => {
                    ...
                    if (canvas) {
                        ...
                        errorDiv.innerHTML = '<p style="color: #e74c3c; text-align: center; padding: 20px;">Chart loading failed. Please refresh the dashboard.</p>';
...
<head>
    ...
    <script>
        // Chart.js loading with fallback
            ...
            return new Promise((resolve, reject) => {
                ...
                script.onerror = () => {
                    ...
                    fallbackScript.onerror = () => {
                        // Final fallback to local version if available
                        const localScript = document.createElement('script');
                        localScript.src = 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.js';
                        localScript.onload = () => resolve();
                        localScript.onerror = () => reject(new Error('Failed to load Chart.js'));
...
`;
    ...
    public async showPreviewPanel(): Promise<void> {
        try {
    ...
    private async updatePreviewContent(tab = 'dashboard'): Promise<void> {
        if (!this.previewPanel) return;

        // Use port 3001 for preview server
        const previewUrl = `http://127.0.0.1:3001/preview?tab=${tab}`;
...
<body>
    <div class="preview-header">
        <div class="tab-selector">
            ...
            <button class="tab-button" onclick="switchTab('cursorrules')">Cursor Rules</button>
    ...
    private setupPreviewHotReload(): void {
