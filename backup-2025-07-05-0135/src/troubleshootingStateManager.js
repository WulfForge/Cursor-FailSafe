...
exports.TroubleshootingStateManager = void 0;
...
class TroubleshootingStateManager {
    constructor(context, logger) {
        ...
        this.maxAttemptsBeforeRestorePoint = 3;
     ...
     */
    startTroubleshooting(issue, description) {
     ...
     */
    recordFileChange(attemptId, filePath, originalContent, newContent, changeType) {
     ...
     */
    completeAttempt(attemptId, status) {
        const attempt = this.findAttempt(attemptId);
        if (!attempt) {
     ...
     */
    async createRestorePoint(issueId, attempt) {
        try {
            ...
            vscode.window.showInformationMessage(`🛡️ Restore point created! This is attempt #${attempt.attemptNumber} for this issue. You can restore to this state if needed.`, 'View Restore Points', 'Dismiss').then(selection => {
    ...
    async showRestorePoints() {
        const panel = vscode.window.createWebviewPanel('restorePoints', 'FailSafe - Restore Points', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        const html = await new commands_1.Commands(this.context).applyCursorRulesToHtml(this.generateRestorePointsHTML());
    ...
    async restoreToPoint(restorePointId) {
        ...
        try {
            // Confirm with user
            const confirm = await vscode.window.showWarningMessage(`Are you sure you want to restore to this point? This will revert ${restorePoint.files.length} files.`, 'Yes, Restore', 'Cancel');
            ...
            vscode.window.showInformationMessage(`✅ Successfully restored to checkpoint from ${restorePoint.timestamp.toLocaleString()}`);
     ...
     */
    async restoreFile(fileChange) {
        try {
            if (fileChange.changeType === 'deleted') {
                // File was deleted, recreate it
                await vscode.workspace.fs.writeFile(vscode.Uri.file(fileChange.filePath), Buffer.from(fileChange.originalContent, 'utf8'));
            }
    ...
    saveState() {
        try {
            ...
            fs.writeFileSync(path.join(this.storagePath, 'state.json'), JSON.stringify(state, null, 2));
        }
        catch (error) {
...
exports.TroubleshootingStateManager = TroubleshootingStateManager;
//# sourceMappingURL=troubleshootingStateManager.js.map