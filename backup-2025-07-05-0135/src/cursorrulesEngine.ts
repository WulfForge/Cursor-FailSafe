...
export interface CursorRule {
    ...
    description?: string;
    pattern: string;
    patternType: 'regex' | 'keyword' | 'semantic' | 'default' | 'command';
    purpose: string;
    severity: 'error' | 'warning' | 'info' | 'critical' | 'safety';
    enabled: boolean;
    createdAt: string;
    updatedAt?: string;
    ...
    triggerCount?: number;
    message?: string;
    response?: 'block' | 'warn' | 'log' | 'suggest' | 'validate' | string;
    scope?: {
        fileTypes: string[];
        projectTypes: string[];
        userRoles: string[];
    ...
    alerting?: {
        whenToAlert: 'immediate' | 'delayed' | 'batch' | 'manual';
        delaySeconds?: number;
        batchSize?: number;
        batchTimeout?: number;
        howToAlert: 'notification' | 'toast' | 'statusbar' | 'log' | 'dashboard' | 'all';
        alertFrequency: 'once' | 'always' | 'throttled';
        throttleMinutes?: number;
        suppressAfterTriggers?: number;
...
export interface Cursorrule {
    ...
    name: string;
    description: string;
    purpose: 'security' | 'quality' | 'compliance' | 'workflow' | 'validate' | 'safety';
    pattern: string;
    patternType: 'regex' | 'keyword' | 'semantic' | 'command';
    response: 'block' | 'warn' | 'log' | 'suggest' | 'validate' | 'validate';
    message: string;
    severity: 'critical' | 'high' | 'medium' | 'low' | 'safety';
    enabled: boolean;
    scope: {
        fileTypes: string[];
        projectTypes: string[];
...
export class CursorrulesEngine {
    ...
    constructor(context: vscode.ExtensionContext, logger: Logger) {
        ...
        // Strict enforcement of non-removable core rules
        this.enforceCoreRules();
        ...
        // Add new rule for preventing premature victory declarations
        this.addPrematureVictoryDeclarationRule();
    }
    ...
    public createRule(ruleData: Omit<CursorRule, 'id' | 'createdAt' | 'updatedAt' | 'usageStats'>): CursorRule {
        const rule: CursorRule = {
            ...ruleData,
            id: this.generateId(),
    ...
    public updateRule(id: string, updates: Partial<CursorRule>): CursorRule | null {
        // Block updates to core rules
        const coreRuleIds = ['core-validate-chat', 'core-workspace-safety'];
        if (coreRuleIds.includes(id)) {
            this.logger.warn(`Attempted to update non-removable core rule: ${id}. Operation blocked.`);
            return null;
        }
        const rule = this.rules.find(rule => rule.id === id);
        if (!rule) return null;

        const updatedRule: CursorRule = {
            ...rule,
            ...updates,
            updatedAt: new Date().toISOString()
        };
    ...
    public deleteRule(id: string): boolean {
        // Block deletion of core rules
        const coreRuleIds = ['core-validate-chat', 'core-workspace-safety'];
        if (coreRuleIds.includes(id)) {
            this.logger.warn(`Attempted to delete non-removable core rule: ${id}. Operation blocked.`);
            return false;
        }
    ...
    public getRule(id: string): CursorRule | undefined {
        return this.rules.find(rule => rule.id === id);
    }

    public getAllRules(): CursorRule[] {
        return this.rules;
    }

    public getEnabledRules(): CursorRule[] {
        return this.rules.filter(rule => rule.enabled);
    }

    public evaluateContent(content: string): RuleMatch[] {
    ...
    private evaluateRule(rule: CursorRule, content: string): RuleMatch {
        let matched = false;
        let match = '';
        let confidence = 0;
    ...
    private getSemanticPatterns(purpose: string): string[] {
    ...
    private findLineNumber(content: string, match: string): number | undefined {
    ...
    private generateSuggestions(rule: CursorRule, match: string): string[] {
    ...
    public recordOverride(ruleId: string): void {
        const rule = this.rules.find(r => r.id === ruleId);
        if (rule) {
            if (!rule.usageStats) {
                rule.usageStats = { triggers: 0, overrides: 0 };
    ...
    public getStats(): { totalRules: number; enabledRules: number; totalTriggers: number; totalOverrides: number } {
        return {
    ...
    public getRulesByPurpose(purpose: string): CursorRule[] {
        switch (purpose) {
            case 'security': {
    ...
    private addDefaultFilesystemRule(): void {
     ...
     */
    private getDefaultProactiveRules(): CursorRule[] {
        return [
            {
                id: this.generateId(),
     ...
     */
    private addDefaultProactiveRules(): void {
     ...
     */
    private addMinimalValidationRule(): void {
     ...
     */
    private addInherentRules(): void {
        const inherentRules: Omit<CursorRule, 'id' | 'createdAt' | 'updatedAt' | 'usageStats'>[] = [
            {
     ...
     * Add workflow automation rules for Vibe Coders
     */
    private addWorkflowAutomationRules(): void {
        const workflowRules: Omit<CursorRule, 'id' | 'createdAt' | 'updatedAt' | 'usageStats'>[] = [
            {
                name: 'Full Verification Process Enforcement',
                pattern: '\\b(?:compile\\s+and\\s+package|package\\s+for\\s+review|build\\s+package|generate\\s+package|create\\s+package|npm\\s+run\\s+package)\\b',
                patternType: 'regex',
     ...
     */
    public getRulesByRole(role: string): CursorRule[] {
    ...
    public async validateText(text: string): Promise<{ violations: ValidationResult[] }> {
        try {
            ...
            for (const rule of enabledRules) {
                ...
                for (const match of ruleMatches) {
                    violations.push({
                        triggered: true,
                        rule: rule as any, // Type conversion for compatibility
                        match: match.match || '',
                        line: match.line,
        ...
        for (const rule of enabledRules) {
            ...
            if (regex.test(revisedHtml)) {
                ...
                switch (rule.name) {
                    ...
                    case 'Full Verification Process Enforcement':
                        // Block incomplete package generation attempts
                        revisedHtml = revisedHtml.replace(regex, (match) => {
                            return `🚨 BLOCKED: ${match} - Use publish.bat or publish.ps1 for full verification pipeline`;
                        });
                        changeLog.push('Blocked incomplete package generation - enforced full verification process');
                        break;
                        
                    case 'Incomplete Build Detection':
                        // Warn about incomplete build processes
                        revisedHtml = revisedHtml.replace(regex, (match) => {
                            return `⚠️ WARNING: ${match} - Missing verification steps. Use complete pipeline.`;
                        });
                        changeLog.push('Warned about incomplete build process - missing verification steps');
                        break;
                        
                    case 'Verification Pipeline Reminder':
                        // Add verification pipeline reminder
                        changeLog.push('Verification pipeline reminder added - ensure complete process');
                        break;
    ...
    public triggerManualAlert(ruleId: string): void {
    ...
    private addPrematureVictoryDeclarationRule(): void {
        const existingRule = this.rules.find(rule => rule.name === 'Prevent Premature Victory Declarations');
        if (!existingRule) {
            this.createRule({
    ...
    // Core rule definitions (must match .cursorrules enforced rules)
    private getCoreRules(): CursorRule[] {
        return [
            {
    ...
    // Enforce core rules: always present, always correct, always first
    private enforceCoreRules(): void {
        const coreRules = this.getCoreRules();
        let changed = false;
        for (let i = 0; i < coreRules.length; i++) {
            const existing = this.rules.find(r => r.name === coreRules[i].name);
            if (!existing || !this.isCoreRuleCorrect(existing, coreRules[i])) {
                // Remove any incorrect or duplicate core rule
                this.rules = this.rules.filter(r => r.name !== coreRules[i].name);
                // Insert correct core rule at the right position
                this.rules.splice(i, 0, { ...coreRules[i], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
                this.logger.warn(`Core rule '${coreRules[i].name}' was missing or altered. Restored to enforced form.`);
                changed = true;
            }
        }
        // Ensure core rules are always first
        this.rules = [
            ...coreRules.map(r => this.rules.find(rule => rule.name === r.name)!),
            ...this.rules.filter(r => !coreRules.some(cr => cr.name === r.name))
        ];
        if (changed) this.saveRules();
    }

    // Check if a core rule matches the enforced definition
    private isCoreRuleCorrect(existing: CursorRule, enforced: CursorRule): boolean {
        return (
            existing.pattern === enforced.pattern &&
            existing.patternType === enforced.patternType &&
            existing.purpose === enforced.purpose &&
            existing.severity === enforced.severity &&
            existing.response === enforced.response &&
