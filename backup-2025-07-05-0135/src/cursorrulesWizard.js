...
exports.CursorrulesWizard = void 0;
const vscode = __importStar(require("vscode"));
const commands_1 = require("./commands");
class CursorrulesWizard {
    ...
    async createRule() {
        const panel = vscode.window.createWebviewPanel('cursorrulesWizard', 'Create Cursorrule', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = await new commands_1.Commands({}).applyCursorRulesToHtml(this.getWizardHtml());
        return new Promise((resolve) => {
            panel.webview.onDidReceiveMessage(async (message) => {
                switch (message.command) {
                    case 'createRule':
                        try {
                            // Create FailSafe rule
                            const rule = this.engine.createRule(message.ruleData);
                            // Create Cursor rule (.mdc file)
                            await this.createCursorRule(message.ruleData);
                            // Update CursorRules file with natural language rule
                            await this.updateCursorRulesFile(message.ruleData);
                            vscode.window.showInformationMessage(`Rule "${rule.name}" created successfully in all systems!`);
                            resolve(rule);
                            panel.dispose();
                        }
                        catch (error) {
    ...
    async editRule(rule) {
        const panel = vscode.window.createWebviewPanel('cursorrulesWizard', `Edit Cursorrule: ${rule.name}`, vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = await new commands_1.Commands({}).applyCursorRulesToHtml(this.getWizardHtml(rule));
        return new Promise((resolve) => {
            panel.webview.onDidReceiveMessage(async (message) => {
                switch (message.command) {
                    case 'createRule':
    ...
    testPattern(pattern, patternType, testContent) {
    ...
    patternTestResultsToMarkdown(testResult) {
...
${r.error}\n`;
    ...
    async createCursorRule(ruleData) {
        const fs = require('fs');
        const path = require('path');
        // Create .mdc file in .cursor/rules/
        const rulesDir = '.cursor/rules';
        const fileName = `${this.sanitizeFileName(ruleData.name)}.mdc`;
        const filePath = path.join(rulesDir, fileName);
        // Create YAML frontmatter
        const yamlFrontmatter = `---
description: |
  ${ruleData.description || ruleData.name}
globs: ["*"]
alwaysApply: false
severity: ${this.mapSeverity(ruleData.severity)}
pattern: "${ruleData.pattern}"
response: ${this.mapResponse(ruleData.response)}
---`;
        // Create markdown content
        const markdownContent = `# ${ruleData.name}

## Purpose
${ruleData.purpose || 'validation'}
...
1. The claim is accurate and verified
2. The pattern matches the intended behavior
3. The response is appropriate for the context

## Response
- **${this.mapSeverity(ruleData.severity)} Level:** ${this.getResponseDescription(ruleData.response)}
- **Action:** ${this.getActionDescription(ruleData.response)}
- **Fallback:** ${this.getFallbackDescription(ruleData.response)}

## Implementation Notes
This rule helps ensure ${ruleData.purpose || 'validation'} follows best practices and maintains quality standards.`;
    ...
    async updateCursorRulesFile(ruleData) {
        const fs = require('fs');
        const path = require('path');
        ...
        if (fs.existsSync(cursorRulesPath)) {
            currentContent = fs.readFileSync(cursorRulesPath, 'utf8');
        }
        else {
            currentContent = `# FailSafe Core Rules
...
`;
        ...
        // Add the new rule
        const newRule = `- ${ruleData.description || ruleData.name} (${ruleData.pattern})`;
        // Find the Custom Rules section or create it
        if (currentContent.includes('## Custom Rules')) {
            // Add to existing Custom Rules section
            currentContent = currentContent.replace('## Custom Rules', `## Custom Rules\n${newRule}`);
        }
        else {
            // Add Custom Rules section at the end
            currentContent += `\n## Custom Rules\n${newRule}\n`;
        }
        fs.writeFileSync(cursorRulesPath, currentContent);
    }
    ...
    mapResponse(response) {
        ...
        return responseMap[response] || 'suggest';
    }
    getResponseDescription(response) {
        ...
        return descriptions[response] || 'Suggestion';
    }
    getActionDescription(response) {
        const actions = {
            'block': 'Stop execution until resolved',
            'warn': 'Show warning to user',
            'log': 'Record in logs',
            'suggest': 'Provide suggestion',
            'validate': 'Require verification'
        };
        return actions[response] || 'Provide suggestion';
    }
    getFallbackDescription(response) {
        const fallbacks = {
            ...
            'suggest': 'Consider the suggestion',
            'validate': 'Manual verification needed'
        };
        return fallbacks[response] || 'Consider the suggestion';
    }
}
exports.CursorrulesWizard = CursorrulesWizard;
//# sourceMappingURL=cursorrulesWizard.js.map