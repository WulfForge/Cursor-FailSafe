...
export class VersionManager {
    ...
    public async getVersionWarningLog(): Promise<any[]> {
        try {
            const logPath = path.join(this.workspaceRoot, '.failsafe', 'version-warnings.json');
            ...
            this.logger.error('Failed to get version warning log', error);
            return [];
        }
    }

    public getVersionLogs(): Array<{timestamp: string; message: string}> {
        try {
            const logPath = path.join(this.workspaceRoot, '.failsafe', 'version-warnings.json');
            if (fs.existsSync(logPath)) {
                const logContent = fs.readFileSync(logPath, 'utf8');
            ...
            this.logger.error('Failed to get version logs', error);
            return [];
        }
    }
} 