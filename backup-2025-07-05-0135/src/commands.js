    ...
    async validateChatMinimal() {
        try {
            ...
            if (issues.length === 0) {
                ...
                vscode.window.showWarningMessage(`⚠️ Minimal validation found ${issues.length} potential issues: ${issues.join(', ')}`);
            }
        }
        catch (error) {
    ...
    async evaluateTechDebt() {
        try {
            ...
            if (analysis.issues.length === 0) {
                ...
                vscode.window.showWarningMessage(`⚠️ Tech debt analysis found ${analysis.issues.length} issues:\n${analysis.issues.join('\n')}`);
            }
        }
        catch (error) {
