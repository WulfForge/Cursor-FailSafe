...
class PreviewInstanceRegistry {
    private static instances = new Map<string, PreviewInstance>();
    private static portRegistry = new Set<number>();
    private static readonly PORT_RANGE = { min: 3000, max: 3100 };
    ...
    static registerInstance(instance: PreviewInstance): void {
        this.instances.set(instance.instanceId, instance);
        this.portRegistry.add(instance.port);
    }

    static unregisterInstance(instanceId: string): void {
    ...
    static getInstance(instanceId: string): PreviewInstance | undefined {
        return this.instances.get(instanceId);
    }

    static getInstancesByWorkspace(workspacePath: string): PreviewInstance[] {
        return Array.from(this.instances.values()).filter(
            instance => instance.workspacePath === workspacePath
        );
    }

    static getAllInstances(): PreviewInstance[] {
        return Array.from(this.instances.values());
    }

    static updateActivity(instanceId: string): void {
        const instance = this.instances.get(instanceId);
        if (instance) {
...
const fastifyPreview: FastifyPluginAsync<PreviewOptions> = async (fastify, options) => {
    ...
    (fastify as any).preview = {
        async generateComponentPreview(componentId: string, options?: Partial<PreviewOptions>): Promise<ComponentPreview> {
            ...
            try {
                ...
                const preview: ComponentPreview = {
                    ...
                    content: await (fastify as any).preview.generateComponentPreviewHTML(component),
        ...
        async startPreviewServer(options?: Partial<PreviewOptions>): Promise<{ port: number; url: string; instanceId: string }> {
        ...
        async generateComponentPreviewHTML(component: any): Promise<string> {
                ...
                switch (component.type) {
                    case 'tab':
                        if (component.name === 'Dashboard') {
                            content = await (fastify as any).preview.generateDashboardContent();
                        } else if (component.name === 'Console') {
                            content = await (fastify as any).preview.generateConsoleContent();
                        } else if (component.name === 'Sprint') {
                            content = await (fastify as any).preview.generateSprintContent();
                        } else if (component.name === 'Cursor Rules') {
                            content = await (fastify as any).preview.generateCursorRulesContent();
                        } else if (component.name === 'Logs') {
                            content = await (fastify as any).preview.generateLogsContent();
                        } else {
        ...
        async getComponentById(componentId: string): Promise<any> {
            try {
                ...
                const projectComponents = await (fastify as any).preview.scanProjectComponents();
                const component = projectComponents.find((c: any) => c.id === componentId);
        ...
        async getUITabs(): Promise<any[]> {
            try {
                ...
                const implementedTabs = projectComponents
                    .filter((comp: any) => comp.type === 'tab' || comp.type === 'component')
                    .map((comp: any) => ({
                        id: comp.id,
                        name: comp.name,
                ...
                implementedTabs.forEach((tab: any) => {
                    if (tab.id !== 'dashboard') {
                        tabs.push(tab);
                    }
                });
        ...
        async getUIComponents(): Promise<any[]> {
            try {
                // Scan the actual project for implemented components
                const projectComponents = await (fastify as any).preview.scanProjectComponents();
                return projectComponents.filter((comp: any) => comp.type === 'component');
            } catch (error) {
        ...
        async getInstanceStatus(): Promise<{ instanceId: string; projectName: string; port: number; url: string; startTime: Date; lastActivity: Date }> {
        ...
        async getAllInstances(): Promise<PreviewInstance[]> {
        ...
        async cleanupInactiveInstances(maxInactiveMinutes: number = 30): Promise<void> {
            ...
            for (const instance of instances) {
                ...
                if (inactiveTime > maxInactiveMs) {
                    await (fastify as any).preview.stopInstance(instance.instanceId);
                }
            }
        },

        async scanProjectComponents(): Promise<any[]> {
            ...
            try {
                        ...
                        if (stats.isDirectory()) {
                            ...
                            for (const file of files) {
                                if (file.endsWith('.ts') || file.endsWith('.tsx') || file.endsWith('.js') || file.endsWith('.jsx') || file.endsWith('.html')) {
                                    ...
                                    const component = await (fastify as any).preview.analyzeComponentFile(componentPath);
                                    if (component) {
                            ...
                            // Analyze single file
                            const component = await (fastify as any).preview.analyzeComponentFile(fullPath);
                            if (component) {
                                components.push(component);
                            }
                        }
                ...
                if (fs.existsSync(uiPath)) {
                    ...
                    const uiMethods = await (fastify as any).preview.analyzeUIMethods(uiContent);
                ...
                if (fs.existsSync(packagePath)) {
                    ...
                    const projectType = await (fastify as any).preview.analyzeProjectType(packageContent);
        ...
        async analyzeComponentFile(filePath: string): Promise<any | null> {
            try {
                ...
                return {
                    ...
                    type: componentType,
                    path: relativePath,
                    content: await (fastify as any).preview.generateComponentPreviewHTML({
                        name: componentName,
                        type: componentType,
                        path: relativePath,
                        hasImplementation
        ...
        async analyzeUIMethods(uiContent: string): Promise<any[]> {
            const components: any[] = [];
        ...
        async analyzeProjectType(packageContent: any): Promise<any> {
        ...
        async generateConsoleContent(): Promise<string> {
            try {
                const uiPath = path.join(currentWorkspacePath, 'src/ui.ts');
                if (fs.existsSync(uiPath)) {
                    const uiContent = fs.readFileSync(uiPath, 'utf8');
        ...
        async generateSprintContent(): Promise<string> {
            try {
                const uiPath = path.join(currentWorkspacePath, 'src/ui.ts');
                if (fs.existsSync(uiPath)) {
                    const uiContent = fs.readFileSync(uiPath, 'utf8');
        ...
        async generateCursorRulesContent(): Promise<string> {
            try {
                const uiPath = path.join(currentWorkspacePath, 'src/ui.ts');
                if (fs.existsSync(uiPath)) {
                    const uiContent = fs.readFileSync(uiPath, 'utf8');
        ...
        async generateLogsContent(): Promise<string> {
            try {
                const uiPath = path.join(currentWorkspacePath, 'src/ui.ts');
                if (fs.existsSync(uiPath)) {
                    const uiContent = fs.readFileSync(uiPath, 'utf8');
        ...
        setupFileWatching(): void {
            try {
                ...
                watcher.on('change', (filePath) => {
                    ...
                    (fastify as any).preview.notifyFileChange(filePath);
                });
                ...
                (fastify as any).preview.fileWatcher = watcher;
        ...
        notifyFileChange(filePath: string): void {
            ...
            if ((fastify as any).preview.connectedClients) {
                (fastify as any).preview.connectedClients.forEach((client: any) => {
                    try {
        ...
        getProjectMetrics(): any {
            try {
                    ...
                    if (fs.existsSync(dir)) {
                        ...
                        files.forEach(file => {
                            ...
                            if (file.isDirectory() && !file.name.startsWith('.')) {
                                ...
                                if (ext === '.ts' || ext === '.tsx') metrics.files.typescript++;
                                else if (ext === '.js' || ext === '.jsx') metrics.files.javascript++;
                                else if (ext === '.html') metrics.files.html++;
                                else if (ext === '.css' || ext === '.scss') metrics.files.css++;
                            }
                        });
                    }
                };
                ...
                metrics.implementation.progress = Math.round(
                    (metrics.components.coverage + (metrics.files.typescript > 0 ? 20 : 0) + (metrics.files.html > 0 ? 10 : 0)) / 3
                );

                if (metrics.implementation.progress >= 90) metrics.implementation.status = 'complete';
                else if (metrics.implementation.progress >= 50) metrics.implementation.status = 'in_progress';
                else metrics.implementation.status = 'starting';
    ...
    fastify.get('/api/preview/status', async (request, reply) => {
        try {
            const status = await (fastify as any).preview.getInstanceStatus();
    ...
    fastify.get('/api/preview/instances', async (request, reply) => {
        const instances = await (fastify as any).preview.getAllInstances();
        return reply.send(instances);
    });

    fastify.delete('/api/preview/stop/:instanceId?', async (request, reply) => {
        const { instanceId } = request.params as { instanceId?: string };
        await (fastify as any).preview.stopInstance(instanceId);
        return reply.send({ success: true });
    });

    fastify.post('/api/preview/cleanup', async (request, reply) => {
        const { maxInactiveMinutes = 30 } = request.body as { maxInactiveMinutes?: number };
        await (fastify as any).preview.cleanupInactiveInstances(maxInactiveMinutes);
        return reply.send({ success: true });
    });
    ...
    fastify.get('/api/project/metrics', async (request, reply) => {
        try {
            const metrics = await (fastify as any).preview.getProjectMetrics();
    ...
    fastify.get('/api/preview/watching', async (request, reply) => {
        try {
            const isWatching = !!(fastify as any).preview.fileWatcher;
            const watchedFiles = isWatching ? (fastify as any).preview.fileWatcher.getWatched() : {};
            const connectedClients = (fastify as any).preview.connectedClients?.length || 0;
    ...
    fastify.post('/api/preview/watch', async (request, reply) => {
        try {
            (fastify as any).preview.setupFileWatching();
    ...
    fastify.post('/api/preview/refresh', async (request, reply) => {
        try {
            ...
            (fastify as any).preview.notifyFileChange('manual-refresh');
