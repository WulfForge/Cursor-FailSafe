...
const fastifyHealth = async (fastify, options) => {
    ...
    fastify.get('/health/detailed', async (request, reply) => {
        ...
        const customHealthResults = await Promise.all(customChecks.map(async (check) => {
            try {
                return await check();
            }
            catch (error) {
...
async function checkPluginHealth(fastify) {
    const checks = [];
...
exports.default = fastifyHealth;
//# sourceMappingURL=fastify-health.js.map