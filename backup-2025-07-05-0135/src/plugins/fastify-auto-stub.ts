...
const fastifyAutoStub: FastifyPluginAsync<AutoStubOptions> = async (fastify, options) => {
    ...
    fastify.decorate('autoStub', {
        ...
        async checkServerFile(): Promise<{ exists: boolean; path: string; content?: string }> {
        ...
        async checkPluginFile(plugin: string): Promise<{ exists: boolean; path: string; content?: string }> {
