...
exports.CursorRulesCommands = void 0;
const vscode = __importStar(require("vscode"));
const aiResponseValidator_1 = require("../aiResponseValidator");
class CursorRulesCommands {
    constructor(logger, cursorrulesEngine, cursorrulesManager, extensionContext) {
        this.logger = logger;
    ...
    async addFromTemplate() {
        try {
            ...
            const selectedTemplate = await vscode.window.showQuickPick(templates.map(t => t.name), {
    ...
    async manageRules() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('manageRules', 'Manage Cursor Rules', vscode.ViewColumn.One, { enableScripts: true });
    ...
    async testPassiveValidation() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('passiveValidationTest', 'Passive Validation Test Results', vscode.ViewColumn.One, { enableScripts: true });
    ...
    async validateWithRules() {
        try {
            ...
            if (result.appliedChanges) {
                vscode.window.showWarningMessage(`⚠️ Cursor rules applied ${result.changeLog.length} changes to the content`);
    ...
    async deleteRule(ruleId) {
        try {
            ...
            const confirm = await vscode.window.showWarningMessage(`Are you sure you want to delete rule "${rule.name}"?`, 'Yes', 'No');
            if (confirm === 'Yes') {
    ...
    async viewRuleStats(ruleId) {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('ruleStats', `Rule Stats: ${rule.name}`, vscode.ViewColumn.One, { enableScripts: true });
    ...
    async showValidationResults(result, fileName) {
        const panel = vscode.window.createWebviewPanel('validationResults', 'Cursor Rules Validation Results', vscode.ViewColumn.One, { enableScripts: true });
    ...
    generatePassiveValidationTestHTML(testResults) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
    ...
    generateValidationResultsHTML(result, fileName) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
...
exports.CursorRulesCommands = CursorRulesCommands;
//# sourceMappingURL=cursorRulesCommands.js.map