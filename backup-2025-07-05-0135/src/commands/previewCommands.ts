...
export class PreviewCommands {
    ...
    private static currentWorkspaceInstance: string | null = null;
    private static workspaceChangeListener: vscode.Disposable | null = null;
    private static autoTrackingEnabled = true;
    ...
    static registerCommands(context: vscode.ExtensionContext) {
        ...
        context.subscriptions.push(
            startPreview,
            startPreviewForWorkspace,
            listPreviewInstances,
            stopPreviewInstance,
            stopAllPreviewInstances,
            openPreviewInBrowser,
            togglePreviewAutoTracking,
            openPreviewInWebview,
            runStartScriptInWebview
        );
    }

    private static setupWorkspaceTracking(context: vscode.ExtensionContext) {
    ...
    private static async toggleAutoTracking(): Promise<void> {
    ...
    private static async startPreviewForCurrentWorkspace(): Promise<void> {
    ...
    private static async startPreviewInstance(workspacePath: string, isCurrentWorkspace: boolean = false): Promise<void> {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const child = spawn('node', ['scripts/preview-manager.js', 'start', '--browser'], {
                    cwd: workspacePath,
                ...
                child.stdout?.on('data', (data) => {
                    ...
                    if (output.includes('Preview server started!') && serverUrl) {
                        ...
                        vscode.window.showInformationMessage(
                            `Preview started for ${projectName} on port ${serverPort}${statusText}`,
                            ...actions
                        ).then(selection => {
    ...
    private static async showActiveInstances(): Promise<void> {
    ...
    private static async stopSpecificInstance(): Promise<void> {
        await this.showActiveInstances();
    }

    private static async stopAllInstances(): Promise<void> {
        if (this.activeInstances.size === 0) {
        ...
        const result = await vscode.window.showWarningMessage(
            `Stop all ${this.activeInstances.size} preview instances?`,
            'Yes',
            'No'
        );

        if (result === 'Yes') {
            const promises = Array.from(this.activeInstances.keys()).map(instanceId => 
    ...
    private static async stopInstance(instanceId: string): Promise<void> {
    ...
    private static async openPreviewInBrowser(): Promise<void> {
        if (this.activeInstances.size === 0) {
    ...
    private static async openInstanceInBrowser(instance: any): Promise<void> {
    ...
    private static async openPreviewInWebview(): Promise<void> {
        // Check if preview server is running
        if (this.activeInstances.size === 0) {
            const startServer = await vscode.window.showInformationMessage(
                'No preview server is running. Would you like to start one?',
    ...
    private static async restartPreviewServer(): Promise<void> {
    ...
    private static async copyInstanceUrl(instance: any): Promise<void> {
        // Try to construct the URL based on common patterns
        const possibleUrls = [
            `http://localhost:3001/preview`,
            `http://localhost:3000/preview`,
    ...
    private static async runStartScriptInWebview(): Promise<void> {
        // Create webview panel
        const panel = vscode.window.createWebviewPanel(
            'failsafeStartScript',
            'FailSafe Preview Server',
            vscode.ViewColumn.Two,
            {
                enableScripts: true,
                retainContextWhenHidden: true
