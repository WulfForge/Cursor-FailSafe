...
class LogCommands {
    ...
    async viewLogs() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('systemLogs', 'System Logs', vscode.ViewColumn.One, { enableScripts: true });
    ...
    async clearLogs() {
        try {
            const confirm = await vscode.window.showWarningMessage('Are you sure you want to clear all logs?', 'Yes', 'No');
            if (confirm === 'Yes') {
    ...
    async exportLogs() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('exportLogs', 'Export Logs', vscode.ViewColumn.One, { enableScripts: true });
    ...
    async monitorSystemHealth() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('systemHealth', 'System Health Monitor', vscode.ViewColumn.One, { enableScripts: true });
    ...
    async debugExtension() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('debugExtension', 'Extension Debug Info', vscode.ViewColumn.One, { enableScripts: true });
    ...
    formatLogsForExport(logs) {
        return logs.map(log => `[${log.timestamp.toISOString()}] ${log.level.toUpperCase()} [${log.source}]: ${log.message}`).join('\n');
    }
    ...
    generateExportLogsHTML(logContent) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
