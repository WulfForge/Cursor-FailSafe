...
export interface SprintTask extends Task {
    ...
    assignee: string; // AI, User, or 3rd party
}
...
export class SprintPlanner {
    ...
    public async updateTask(taskId: string, updatedTask: Partial<SprintTask>): Promise<boolean> {
        if (!this.currentSprint) {
     ...
     */
    public async exportSprintToJSON(exportType: 'full_sprint' | 'partial_sprint' | 'single_task' = 'full_sprint', taskId?: string): Promise<string> {
        try {
     ...
     */
    public async importSprintFromJSON(jsonData: string): Promise<{ success: boolean; message: string; importedData?: any }> {
        try {
     ...
     */
    private async importSingleTask(taskData: any): Promise<{ success: boolean; message: string; importedData?: any }> {
        try {
            if (!this.currentSprint) {
     ...
     */
    private async importPartialSprint(sprintData: any): Promise<{ success: boolean; message: string; importedData?: any }> {
        try {
            if (!this.currentSprint) {
            ...
            if (updatedSprint.name) this.currentSprint.name = updatedSprint.name;
            if (updatedSprint.description) this.currentSprint.description = updatedSprint.description;
            if (updatedSprint.startDate) this.currentSprint.startDate = updatedSprint.startDate;
            if (updatedSprint.endDate) this.currentSprint.endDate = updatedSprint.endDate;
            if (updatedSprint.duration) this.currentSprint.duration = updatedSprint.duration;
            if (updatedSprint.status) this.currentSprint.status = updatedSprint.status;
            if (updatedSprint.velocity) this.currentSprint.velocity = updatedSprint.velocity;
            if (updatedSprint.capacity) this.currentSprint.capacity = updatedSprint.capacity;
            if (updatedSprint.teamSize) this.currentSprint.teamSize = updatedSprint.teamSize;
            if (updatedSprint.metadata) this.currentSprint.metadata = { ...this.currentSprint.metadata, ...updatedSprint.metadata };
     ...
     */
    private async importFullSprint(sprintData: any): Promise<{ success: boolean; message: string; importedData?: any }> {
        try {
     ...
     */
    private validateImportFormat(data: any): { isValid: boolean; error?: string } {
     ...
     */
    private serializeSprint(sprint: SprintPlan): any {
        return {
            id: sprint.id,
            name: sprint.name,
     ...
     */
    private serializeSprintPartial(sprint: SprintPlan): any {
        return {
            name: sprint.name,
            description: sprint.description,
            startDate: sprint.startDate.toISOString(),
     ...
     */
    private serializeTask(task: SprintTask): any {
        return {
            id: task.id,
            name: task.name,
            description: task.description,
     ...
     */
    private deserializeSprint(sprintData: any): SprintPlan {
        return {
            ...
            tasks: sprintData.tasks.map((taskData: any) => this.deserializeTask(taskData))
        };
    }
     ...
     */
    private deserializeSprintPartial(sprintData: any): any {
        return {
            name: sprintData.name,
            description: sprintData.description,
     ...
     */
    private deserializeTask(taskData: any): SprintTask {
        return {
     ...
     */
    private findTaskById(taskId: string): SprintTask | null {
