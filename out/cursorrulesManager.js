...
class CursorrulesManager {
        ...
        if (action) {
            switch (action.value) {
                ...
                case 'delete': {
                    ...
                    const selectedRule = await this.selectRule(rules);
                    if (selectedRule) {
                        const confirmed = await vscode.window.showWarningMessage(`Are you sure you want to delete the rule "${selectedRule.name}"?`, 'Yes', 'No');
                        if (confirmed === 'Yes') {
    ...
    async createRule() {
        try {
            ...
            const wizard = new cursorrulesWizard_1.CursorrulesWizard(this.engine, this.logger, this.context);
    ...
    async showRulesList() {
        ...
        const panel = vscode.window.createWebviewPanel('cursorrulesList', 'Cursorrules List', vscode.ViewColumn.One, {});
        ...
        `).join('');
        panel.webview.html = await new commands_1.Commands({}).applyCursorRulesToHtml(`
            <!DOCTYPE html>
            <html>
            <head>
                <style>
    ...
    async showManager() {
        const panel = vscode.window.createWebviewPanel('cursorrulesManager', 'Manage Cursorrules', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = await new commands_1.Commands({}).applyCursorRulesToHtml(this.getManagerHtml());
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'deleteRule': {
                    ...
                    if (ruleToDelete) {
                        const confirmed = await vscode.window.showWarningMessage(`Are you sure you want to delete the rule "${ruleToDelete.name}"?`, 'Yes', 'No');
                        if (confirmed === 'Yes') {
    ...
    showRuleDetails(rule) {
        const panel = vscode.window.createWebviewPanel('cursorruleDetails', `Rule: ${rule.name}`, vscode.ViewColumn.One, { enableScripts: true });
