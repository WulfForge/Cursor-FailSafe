...
exports.RealChartDataService = void 0;
const types_1 = require("./types");
class RealChartDataService {
    constructor(taskEngine, projectPlan, logger) {
        this.actionLog = [];
    ...
    async getProgressData() {
        try {
            ...
            const inProgressTasks = allTasks.filter(task => task.status === types_1.TaskStatus.inProgress);
            const blockedTasks = allTasks.filter(task => task.status === types_1.TaskStatus.blocked);
            const notStartedTasks = allTasks.filter(task => task.status === types_1.TaskStatus.notStarted);
            return {
    ...
    async getActivityData(days = 7) {
        try {
            ...
            for (let i = days - 1; i >= 0; i--) {
                ...
                const dayActions = this.actionLog.filter(action => action.timestamp.startsWith(dateStr)).length;
    ...
    async getPerformanceData() {
        try {
            ...
            const overdueTasks = allTasks.filter(task => task.dueDate && new Date() > task.dueDate && task.status !== types_1.TaskStatus.completed);
    ...
    async getIssuesData() {
        try {
            const allTasks = this.projectPlan.getAllTasks();
            const blockedTasks = allTasks.filter(task => task.status === types_1.TaskStatus.blocked);
            const technicalIssues = blockedTasks.filter(task => task.blockers.some(blocker => blocker.toLowerCase().includes('technical') ||
                blocker.toLowerCase().includes('bug') ||
                blocker.toLowerCase().includes('error'))).length;
            const dependencies = blockedTasks.filter(task => task.blockers.some(blocker => blocker.toLowerCase().includes('dependency') ||
                blocker.toLowerCase().includes('waiting'))).length;
            const resourceConstraints = blockedTasks.filter(task => task.blockers.some(blocker => blocker.toLowerCase().includes('resource') ||
                blocker.toLowerCase().includes('time') ||
                blocker.toLowerCase().includes('budget'))).length;
            const requirementsChanges = blockedTasks.filter(task => task.blockers.some(blocker => blocker.toLowerCase().includes('requirement') ||
                blocker.toLowerCase().includes('spec') ||
                blocker.toLowerCase().includes('change'))).length;
            return {
    ...
    async getHallucinationSourceData() {
        try {
    ...
    getDefaultHallucinationSourceData() {
        return {
...
exports.RealChartDataService = RealChartDataService;
