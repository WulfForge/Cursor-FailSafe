...
if (!content.includes('public async showDashboard(): Promise<void>')) {
    ...
    fs.writeFileSync('src/commands.ts', content, 'utf8');
    console.log('✅ Added showDashboard method and placeholder methods');
}

// Fix 3: Fix ChatValidationContext
content = fs.readFileSync('src/commands.ts', 'utf8');
content = content.replace(
...
// Fix 4: Remove all applyCursorRulesToHtml calls since they're not needed
content = fs.readFileSync('src/cursorrulesManager.ts', 'utf8');
content = content.replace(
    /panel\.webview\.html = await new Commands\([^)]+\)\.applyCursorRulesToHtml\(/g,
    'panel.webview.html = '
);
...
content = content.replace(
    /panel\.webview\.html = await this\.commands\.applyCursorRulesToHtml\(/g,
    'panel.webview.html = '
);
...
content = content.replace(
    /const html = await new Commands\([^)]+\)\.applyCursorRulesToHtml\(/g,
    'const html = '
);
...
console.log('✅ Removed applyCursorRulesToHtml calls');
...
fs.writeFileSync('src/extension.ts', content, 'utf8');
console.log('✅ Fixed extension.ts constructor call');

// Fix 6: Add missing imports to all files
const files = [
...
console.log('✅ Added missing imports to all files');
